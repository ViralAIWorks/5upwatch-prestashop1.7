...
$(document).ready(function() {
    ...
    $(function() {
        var block_scroll_min = 700;
        var show_block_scroll_min = function() {
            var scroll_top_top1 = $(window).scrollTop();
            if (scroll_top_top1 > block_scroll_min) {
                $('#index #right-column .block').addClass('show_block');
            } else {
                $('#index #right-column .block').removeClass('show_block');
            }

        };

        show_block_scroll_min();
    ...
    $(function() {
        var block_scroll_max = 2200;
        var show_block_scroll = function() {
            var scroll_top_top = $(window).scrollTop();
            if (scroll_top_top > block_scroll_max) {
                $('#index #right-column .block').removeClass('show_block');
            } else {
                // $('#index #right-column .block').addClass('show_block');
            }
...
function wcInitImageZoom() {
    ...
    if ($('div:not(.quickview) .images-container-img-sync .product-cover-zoom').length > 0) {
        ...
        var img = $('div:not(.quickview) .product-images-big .product-cover-zoom'),
            img_src1 = $('div:not(.quickview) .product-images-big .product-cover-zoom').data('src');
        img.zoom({
            touch: false,
            url: img_src1
        });
